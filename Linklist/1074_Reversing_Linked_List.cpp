...
using namespace std;
struct node {
    int address, data, next, order = MAX; // 结点在链表中的序号
} list[100020];
// 链表结构本身不需要逆转，只需要从每组最后一个结点开始倒着输出每一块的地址域
// 直到这块的首结点需要做特殊处理，即最后一个结点的next域需要根据当前组数判断是否正常输出下一块的最后一个结点的地址域
int main() {
    int start, N, K;
    cin >> start >> N >> K;
    ...
    for (int p = start; p != -1; p = list[p].next)
        list[p].order = cnt++;
    sort(list, list + 100020, [](const node &a, const node &b) { return a.order < b.order; }); // 不在链表上的无效结点排在最后
