...
int main( ) {
    ...
    for( int i = 0; i < N - 1; i++ ) { // 如果一条记录和它后面紧邻的一条记录符合条件的话
        if( ( origin[i].name == origin[i + 1].name ) && ( origin[i].tag == true && origin[i + 1].tag == false ) ) {
            ...
            temp.end_date = origin[i + 1].date, temp.end_hour = origin[i + 1].hour, temp.end_minute = origin[i + 1].minute;
            temp.time = ( ( temp.end_date - 1 ) * 60 * 24 + temp.end_hour * 60 + temp.end_minute ) - ( ( temp.start_date - 1 ) * 60 * 24 + temp.start_hour * 60 + temp.start_minute );
    ...
    for( map<string, client>::iterator it = user.begin( ); it != user.end( ); it++ ) {
        ...
        for( int i = 0; i < it->second.b.size( ); i++ ) {
            printf( "%02d:%02d:%02d %02d:%02d:%02d %d $%.2lf\n", it->second.b[i].start_date, it->second.b[i].start_hour, it->second.b[i].start_minute,
                it->second.b[i].end_date, it->second.b[i].end_hour, it->second.b[i].end_minute, it->second.b[i].time, it->second.b[i].cost );
        }
